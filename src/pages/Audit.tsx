...
const initialAudits: Audit[] = [
  {
    ...
    items: [
      {
        id: "AST-1001",
        assetName: "Dell XPS 15 - Dev Lab",
        category: "Laptops",
        lastVerified: "15 Apr 2024",
        status: "Verified",
      },
      {
        id: "AST-1002",
        assetName: "MacBook Pro M3 - UI Team",
        category: "Laptops",
        lastVerified: "Now",
        status: "Verified",
      },
      {
        id: "AST-1003",
        assetName: "Sony Alpha A7 - Marketing",
        category: "Equipment",
  ...
  },
  {
    ...
    items: [
      {
        id: "AST-9001",
        assetName: "Company Vehicle V-12",
        category: "Vehicles",
        lastVerified: "12 May 2024",
        status: "Verified",
      },
      {
        id: "AST-9005",
        assetName: "Server Rack Alpha",
        category: "Infrastructure",
        lastVerified: "11 May 2024",
        status: "Verified",
      },
    ],
  },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">
            Verify asset accuracy and regulatory compliance benchmarks
          </p>
        </div>
        <button
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-black text-slate-900 dark:text-white group-hover:text-brand transition-colors">
                      {audit.title}
                    </span>
          ...
          <AuditModal
            ...
            onSave={(a: any) => {
              setAudits((prev) => [
                {
                  ...a,
                  id: `AUD-${Math.floor(Math.random() * 900) + 300}`,
                  status: "Pending",
                  completion: 0,
                  items: [],
                },
...
function ComplianceCard({
  ...
  return (
    <div className="bg-card-bg border border-border-dim p-8 rounded-[2rem] shadow-xl relative overflow-hidden group">
      ...
      <div className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1 relative z-10">
        {title}
      </div>
      <div className="text-4xl font-black text-slate-900 dark:text-white tracking-tight relative z-10">
        {value}
      </div>
      {sub && (
        <div className="text-[10px] text-slate-500 font-bold mt-2 uppercase tracking-widest opacity-60">
          {sub}
        </div>
      )}
    </div>
        ...
        <div className="p-10 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim shadow-inner cursor-pointer"
              >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim shadow-inner"
          ...
          <div className="pt-4">
            <button
              ...
              className="w-full py-5 bg-brand text-white font-black rounded-2xl shadow-2xl shadow-brand/30 active:scale-95 transition-all uppercase tracking-[0.2em] text-xs"
            >
              DEPLOY AUDIT PROTOCOL
            </button>
            <p className="text-center text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-4 opacity-60">
              System will automatically assign auditors for this batch
            </p>
          </div>
        </div>
      </motion.div>
...
function AuditExecutionModal({
  ...
  const performScan = () => {
    ...
    setTimeout(() => {
      ...
      if (pendingIndex > -1) {
        ...
        const verifiedCount = newItems.filter(
          (i) => i.status === "Verified",
        ).length;
  ...
  const markMissing = (id: string) => {
    const newItems = items.map((item) =>
      item.id === id
        ? { ...item, status: "Missing" as const, lastVerified: "Manual Flag" }
        : item,
    );
    setItems(newItems);
    const verifiedCount = newItems.filter(
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-app-bg border border-border-dim w-full max-w-5xl h-full sm:h-[85vh] rounded-none sm:rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col relative"
      >
        {/* Absolute Close Button */}
        <button
          onClick={onClose}
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div>
              <div className="flex items-center gap-3">
                <div className="text-[8px] sm:text-[10px] font-black text-brand uppercase tracking-widest font-mono truncate max-w-[100px] sm:max-w-none">
                  EXECUTION: {audit.id}
                </div>
                <span className="px-2 py-0.5 bg-brand/10 text-brand rounded text-[8px] sm:text-[9px] font-black uppercase tracking-widest">
                  {audit.status}
                </span>
              </div>
              <h3 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight leading-none mt-1">
                {audit.title}
              </h3>
              <p className="text-[8px] sm:text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-2">
                {audit.location} • {audit.type}
              </p>
            </div>
          </div>
            ...
            <button
              ...
              className={cn(
                ...
                scanning
                  ...
                  : "bg-brand text-white shadow-2xl shadow-brand/30 hover:scale-105 active:scale-95",
              )}
            >
              {scanning ? (
                ...
                <>
                  ...
                  <span className="truncate">START SCANNER</span>
                </>
              )}
            </button>
          </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
              ...
              >
                <div className="relative w-full max-w-xs aspect-square">
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center gap-4 sm:gap-6 text-center">
                    ...
                    <div className="space-y-2">
                      <p className="text-lg sm:text-xl font-black uppercase tracking-[0.2em] animate-pulse">
                        Scanning...
                      </p>
                      <p className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        Searching for asset tags
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-10 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                Verification Checklist ({items.length} assets)
              </h4>
              <div className="flex gap-2">
                <span className="px-2 py-1 bg-emerald-500/10 text-emerald-500 text-[8px] sm:text-[10px] font-black rounded uppercase tracking-widest">
                  Verified:{" "}
                  {items.filter((i) => i.status === "Verified").length}
                </span>
                <span className="px-2 py-1 bg-rose-500/10 text-rose-500 text-[8px] sm:text-[10px] font-black rounded uppercase tracking-widest">
            ...
            {items.length === 0 ? (
              <div className="bg-slate-50 dark:bg-white/5 border-2 border-dashed border-border-dim rounded-[1.5rem] sm:rounded-[2.5rem] p-10 sm:p-20 text-center flex flex-col items-center">
                ...
                <h3 className="text-lg sm:text-xl font-black uppercase text-slate-400">
                  Empty Scope
                </h3>
                <p className="text-slate-500 text-[10px] sm:text-xs mt-1 max-w-xs">
                  There are no assets assigned to this specific audit batch yet.
                </p>
                <button className="mt-6 sm:mt-8 px-6 sm:px-8 py-3 bg-app-bg border border-border-dim text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-brand hover:text-white transition-all border-brand/20">
                  Assign Assets
                </button>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-4 sm:gap-5">
                      ...
                      <div className="min-w-0">
                        <div className="text-[8px] sm:text-[10px] font-black text-brand uppercase tracking-tighter font-mono truncate">
                          {item.id}
                        </div>
                        <h5 className="font-black text-slate-900 dark:text-white uppercase tracking-tight text-sm sm:text-base truncate">
                          {item.assetName}
                        </h5>
                        <p className="text-[8px] sm:text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1 truncate">
                          L.V: {item.lastVerified} • {item.category}
                        </p>
                      </div>
                    </div>
                          ...
                          <button
                            onClick={() => {
                              ...
                              const completion = Math.round(
                                (verifiedCount / newItems.length) * 100,
                              );
                              onUpdate({
                                ...audit,
                                items: newItems,
          ...
          <div className="w-full lg:w-96 bg-slate-50 dark:bg-black/20 border-t lg:border-t-0 lg:border-l border-border-dim p-10 space-y-10">
              ...
              <div className="relative w-40 h-40 sm:w-48 sm:h-48 mx-auto">
                <svg className="w-full h-full" viewBox="0 0 100 100">
                  ...
                  <motion.circle
                    ...
                    strokeDasharray="283"
                    animate={{
                      strokeDashoffset: 283 - (283 * audit.completion) / 100,
                    }}
            ...
            <div className="space-y-4 hidden sm:block">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div
                    key={i}
                    className="flex items-center justify-between p-4 bg-app-bg border border-border-dim rounded-2xl shadow-sm"
                  >
            ...
            <div className="bg-brand/10 border border-brand/20 p-6 rounded-[2rem] space-y-3">
              <h5 className="text-[10px] font-black text-brand uppercase tracking-widest">
                Compliance Tip
              </h5>
              <p className="text-xs font-bold text-brand leading-relaxed">
                "Ensure all physical tags are visible. If a tag is damaged, use
                the 'Flag Damaged' action from the item menu."
              </p>
            </div>
          </div>
        </div>
