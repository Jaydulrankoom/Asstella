...
const initialWarranties: Warranty[] = [
  {
    ...
    coverage: "Hardware & Accidental damage",
  },
  {
    id: "WR-105",
    assetName: "Dell Latitude 5420",
    assetCode: "A003",
    provider: "Dell Support Assist",
    type: "Standard",
    status: "Expiring Soon",
    expiryDate: "10 June 2024",
  ...
  },
  {
    ...
    assetCode: "A005",
    provider: "Epson Service",
    type: "Manufacturer",
    status: "Expired",
    expiryDate: "01 May 2024",
    coverage: "Lamp & Optics",
  },
  {
    id: "WR-309",
    ...
    coverage: "Next Business Day Replacement",
  },
  {
    id: "WR-412",
    assetName: "LG Industrial AC",
    assetCode: "AC-05",
    provider: "LG Enterprise",
    type: "Manufacturer",
    status: "Claimed",
    expiryDate: "15 Nov 2024",
    coverage: "Compressor Warranty",
  },
];
...
export default function WarrantyPage() {
  ...
  const handleFileClaim = (warrantyId: string) => {
    setWarranties((prev) =>
      prev.map((w) => (w.id === warrantyId ? { ...w, status: "Claimed" } : w)),
    );
  };
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Monitor coverage status and service level agreements
          </p>
        </div>
        <button
            ...
            >
              ...
              <div className="flex items-center justify-between pt-3 border-t border-border-dim/50">
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-black text-slate-900 dark:text-white uppercase font-mono">
                    {warranty.assetCode}
                  </span>
                </div>
        ...
        {filteredWarranties.length === 0 && (
          <div className="p-20 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-slate-400 font-medium">
              No warranty records matched your search criteria.
            </p>
          </div>
        )}
      </div>
          ...
          <AddWarrantyModal
            ...
            onSave={(w) => {
              setWarranties((prev) => [
                {
                  ...w,
                  id: `WR-${Math.floor(Math.random() * 900) + 500}`,
                  status: "Active",
                },
                ...prev,
              ]);
...
function StatCard({ label, value, icon: Icon, border, text }: any) {
  return (
    <div
      className={cn(
        "bg-card-bg border p-6 rounded-3xl shadow-sm transition-all hover:translate-y-[-2px]",
        border,
      )}
    >
      <div
        className={cn(
          "w-10 h-10 rounded-xl flex items-center justify-center mb-4 bg-app-bg shadow-sm",
          text,
        )}
      >
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-app-bg border border-border-dim w-full max-w-xl rounded-3xl overflow-hidden shadow-2xl"
      >
        ...
        <div className="p-8 space-y-4">
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                className="w-full bg-card-bg border border-border-dim p-4 rounded-2xl text-sm"
          ...
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                className="w-full bg-card-bg border border-border-dim p-4 rounded-2xl text-sm"
                placeholder="Provider (e.g. Apple)"
              />
              <select
                value={formData.type}
                onChange={(e) =>
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              className="w-full bg-card-bg border border-border-dim p-4 rounded-2xl text-sm min-h-[80px]"
              placeholder="Summary of what's covered..."
            />
          </div>
          <button
            onClick={() => onSave(formData)}
            className="w-full py-4 bg-brand text-white font-black rounded-2xl mt-4 uppercase tracking-widest shadow-xl shadow-brand/20 active:scale-95 transition-all"
          >
...
function WarrantyPreviewModal({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4 text-slate-900 dark:text-white"
    >
      <motion.div
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.9, opacity: 0 }}
        className="bg-app-bg border border-border-dim w-full max-w-xl rounded-none sm:rounded-[2.5rem] overflow-hidden shadow-2xl relative h-full sm:h-auto overflow-y-auto"
      >
        <AnimatePresence mode="wait">
            ...
            >
              <div className="p-6 sm:p-8 pb-0 flex flex-col items-center">
                ...
                <p className="text-slate-500 font-bold uppercase text-[9px] sm:text-[10px] tracking-widest mt-1">
                  Ref: {warranty.assetCode} • ID: {warranty.id}
                </p>
              </div>
              ...
              <div className="p-6 sm:p-8 space-y-6">
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <div className="bg-card-bg border border-border-dim p-4 rounded-xl sm:rounded-2xl">
                    <label className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">
                      Status
                    </label>
                    <StatusBadge status={warranty.status} />
                  </div>
                  <div className="bg-card-bg border border-border-dim p-4 rounded-xl sm:rounded-2xl">
                    ...
                    <span className="text-xs sm:text-sm font-black uppercase truncate block">
                      {warranty.provider}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-5 bg-slate-50 dark:bg-white/5 rounded-2xl border border-border-dim relative overflow-hidden">
                    <div className="z-10">
                      ...
                      <span className="text-lg sm:text-xl font-black text-slate-900 dark:text-white uppercase">
                        {warranty.expiryDate}
                      </span>
                    </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1 font-mono uppercase">
                      Scope of Coverage
                    </label>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-6 pb-4">
                  ...
                  <button
                    onClick={onClose}
                    className="w-full sm:flex-1 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-2xl uppercase tracking-widest shadow-xl active:scale-95 transition-all text-[10px] sm:text-xs"
                  >
                    {warranty.status === "Claimed" ||
                    warranty.status === "Expired"
                      ? "CLOSE PREVIEW"
          ...
          {step === "claim" && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="p-6 sm:p-8 space-y-6 h-full sm:h-auto"
            >
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-border-dim">
                  <p className="text-[9px] text-slate-500 font-bold leading-relaxed uppercase">
                    DAMAGES MUST BE WITHIN COVERAGE SCOPE. MISUSE MAY VOID
                    WARRANTY.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4">
                ...
                <button
                  ...
                  className="w-full sm:px-8 py-4 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 font-black rounded-2xl uppercase tracking-widest text-[10px] sm:text-xs"
                >
                  GO BACK
                </button>
              </div>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-500 text-xs sm:text-sm font-medium">
                  Request logged. Provider notified. <br />
                  <span className="font-mono text-brand mt-2 block uppercase">
              ...
              <button
                onClick={onClose}
                className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-2xl uppercase tracking-widest shadow-xl text-xs"
              >
                FINISH
              </button>
            </motion.div>
