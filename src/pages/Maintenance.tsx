...
const initialTickets: Ticket[] = [
  ...
  {
    id: "T-1002",
    assetName: "Generator Honda 5kVA",
    assetCode: "G-125",
    type: "Breakdown",
    priority: "Critical",
    status: "In-Progress",
    date: "22 May 2024",
    assignedTo: "Anil Thompson",
  },
  {
    id: "T-1003",
    ...
    status: "Pending",
    date: "24 May 2024",
    assignedTo: "David Bek",
  },
  {
    id: "T-1004",
    assetName: "Cisco Switch Core",
    assetCode: "N-402",
    type: "Preventive",
    priority: "High",
    status: "In-Progress",
  ...
  {
    id: "T-1005",
    assetName: "Air Conditioner LG 2T",
    assetCode: "AC-05",
    type: "Breakdown",
    priority: "High",
    status: "Pending",
    date: "26 May 2024",
    assignedTo: "Rauf Ahmed",
  },
  {
    id: "T-1006",
    assetName: "Biometric Scanner",
    assetCode: "S-102",
    type: "Calibration",
    priority: "Low",
    status: "Completed",
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Track asset health and service requests
          </p>
        </div>
        <button
      ...
      <div className="bg-card-bg border border-border-dim rounded-2xl p-4 sm:p-5 flex flex-col xl:flex-row xl:items-center gap-4 shadow-sm transition-colors duration-300">
        <div className="w-full xl:flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-app-bg border border-border-dim pl-10 pr-4 py-3 rounded-xl text-xs outline-none focus:ring-2 focus:ring-brand/20 transition-all"
          />
        </div>
        <div className="flex items-center gap-3 w-full xl:w-auto">
          <button className="flex-1 xl:flex-none p-3 bg-app-bg border border-border-dim rounded-xl text-slate-400 hover:text-brand transition-all flex items-center justify-center gap-2">
            ...
            <span className="xl:hidden text-[10px] font-black uppercase tracking-widest">
              Filter
            </span>
          </button>
        </div>
        ...
        {tickets.map((ticket) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-brand font-black text-[10px] uppercase">
                {ticket.id}
              </span>
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-mono uppercase transition-colors">
                {ticket.assetCode}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-brand/20 flex items-center justify-center text-[10px] text-brand">
                  SA
                </div>
            ...
            <tbody className="divide-y divide-border-dim">
              {tickets.map((ticket) => (
                <tr
                  ...
                  onClick={() => setSelectedTicket(ticket)}
                >
                  <td className="px-6 py-4 font-bold text-brand text-xs">
                    {ticket.id}
                  </td>
          ...
          <CreateTicketModal
            ...
            onSave={(t: any) => {
              setTickets((prev) => [
                {
                  ...t,
                  id: `T-${Math.floor(Math.random() * 1000) + 2000}`,
                  status: "Pending",
    ...
    >
      ...
      >
        <div className="p-6 sm:p-8 border-b border-border-dim flex items-center justify-between bg-slate-50/50 dark:bg-white/5">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-brand transition-colors p-2 text-2xl font-bold"
          >
            ×
          </button>
        </div>
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 py-6 border-y border-border-dim/50">
            <div className="space-y-4">
              <div>
                ...
                <div className="text-base sm:text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">
                  {ticket.assetName}
                </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed bg-slate-50 dark:bg-white/5 p-4 sm:p-5 rounded-2xl italic font-medium border border-border-dim/50 uppercase">
              ...
              {ticket.type === "Breakdown"
                ? "Asset reported non-functional by user. Initial diagnostics suggest power supply failure. Spare parts ordered and awaiting arrival."
                : ticket.type === "Preventive"
                  ? "Routine inspection as per manufacturer guideline. Cleaning and optimization performed. Next service due in 6 months."
          ...
          <button
            onClick={onClose}
            className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black rounded-2xl mt-4 uppercase tracking-widest shadow-xl transition-all active:scale-95 text-xs"
          >
            CLOSE PREVIEW
          </button>
        </div>
...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return (
    <span
      className={cn(
        "px-2 py-0.5 rounded text-[10px] font-black uppercase border border-current/20",
        styles[priority],
      )}
    >
      {priority}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={cn(
        "px-2 py-0.5 rounded-full text-[10px] font-bold border border-current/20",
        styles[status],
      )}
    >
      {status}
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.9, opacity: 0 }}
        className="bg-app-bg border border-border-dim w-full max-w-lg rounded-none sm:rounded-3xl overflow-hidden shadow-2xl relative h-full sm:h-auto overflow-y-auto"
      >
        <div className="p-6 sm:p-8 border-b border-border-dim flex items-center justify-between bg-slate-50/50 dark:bg-white/5">
          <h3 className="font-black text-lg sm:text-xl uppercase tracking-tight">
            Create Ticket
        ...
        <div className="p-6 sm:p-8 space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-card-bg p-4 rounded-xl sm:rounded-2xl text-sm font-bold border border-border-dim outline-none focus:ring-1 focus:ring-brand/40 uppercase"
          ...
          <button
            ...
            className="w-full py-4 sm:py-5 bg-brand text-white font-black rounded-xl sm:rounded-2xl mt-4 uppercase tracking-[0.2em] shadow-xl shadow-brand/20 active:scale-95 transition-all text-xs"
          >
            SUBMIT TICKET
          </button>
        </div>
