...
const initialNotifications: Notification[] = [
  { id: 1, title: "Urgent Maintenance", description: "Generator A-12 requires immediate oil change and filter replacement based on running hours.", type: "urgent", time: "2m ago", isRead: false, category: "Maintenance" },
  { id: 2, title: "AMC Expiring Soon", description: "The Dell Support contract (AMC-2024-05) expires in 14 days. Please review renewal terms.", type: "warning", time: "1h ago", isRead: false, category: "Contract" },
  { id: 3, title: "Asset Audited Successfully", description: "Workstation 405 was successfully verified by the floor manager during the weekly scan.", type: "success", time: "3h ago", isRead: true, category: "Audit" },
  { id: 4, title: "New Asset Shared", description: "The HR department shared 10 new Laptops with your department. Please accept and tag them.", type: "info", time: "5h ago", isRead: true, category: "Asset" },
  { id: 5, title: "Warranty Claim Approved", description: "The claim for MacBook Air (M1-129) has been approved. Replacement parts are on the way.", type: "success", time: "1d ago", isRead: true, category: "Warranty" },
  { id: 6, title: "System Security Alert", description: "A new login was detected for your account from a new IP: 192.168.1.45.", type: "warning", time: "2d ago", isRead: true, category: "Security" },
  { id: 7, title: "GPS Tracker Offline", description: "Vehicle DL-36-CA-1002 has lost signal for over 4 hours. Last known location: Sector 5 Industrial Area.", type: "urgent", time: "2d ago", isRead: true, category: "GPS" },
];
...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in slide-in-from-bottom-4 duration-500 p-2 sm:p-0" id="notifications_page">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black tracking-tight flex items-center gap-3">
             <Bell className="w-7 h-7 sm:w-8 sm:h-8 text-brand" /> Notifications Center
          </h2>
          <p className="text-slate-500 font-bold uppercase text-[9px] sm:text-[10px] tracking-widest mt-1">Manage your alerts and system updates</p>
        </div>
                  ...
                  >
                    ...
                    {f.id !== "unread" && (
                      <span className="text-[9px] sm:text-[10px] opacity-60 ml-2">
                        {notifications.filter(n => f.id === "all" ? true : n.type === f.id).length}
                      </span>
                    )}
                    {f.id === "unread" && (
        ...
        <div className="lg:col-span-3 space-y-4">
           {filteredNotifications.length === 0 ? (
             <div className="bg-card-bg border border-border-dim rounded-[1.5rem] sm:rounded-[2rem] p-10 sm:p-20 flex flex-col items-center text-center">
                ...
                <h3 className="text-lg sm:text-xl font-black text-slate-400">No Notifications Found</h3>
                <p className="text-slate-500 text-xs sm:text-sm mt-1 max-w-xs">There are no alerts matching your current filter criteria.</p>
             </div>
           ) : (
                 ...
                 <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                    <div className={cn(
                      ...
                      "bg-brand text-white"
                       ...
                       {notif.type === "urgent" ? <AlertTriangle className="w-6 sm:w-8 h-6 sm:h-8" /> :
                        notif.type === "warning" ? <AlertTriangle className="w-6 sm:w-8 h-6 sm:h-8" /> :
                        notif.type === "success" ? <CheckCircle2 className="w-6 sm:w-8 h-6 sm:h-8" /> :
                    ...
                    <div className="flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-1">
                         <div className="flex items-center gap-3">
                            <h3 className={cn(
                               ...
                               !notif.isRead ? "text-slate-900 dark:text-white" : "text-slate-500"
                            )}>
                               {notif.title}
                            </h3>
                            {!notif.isRead && (
                         ...
                         <div className="flex items-center justify-between sm:justify-end gap-2">
                             <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">{notif.time}</span>
                      ...
                      <p className="text-slate-500 text-xs sm:text-sm font-medium leading-relaxed max-w-2xl">{notif.description}</p>
