...
export default function VendorsPage() {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Are you sure you want to remove this vendor? All history will be archived.")) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500" id="vendors_page">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">Strategic sourcing and vendor relationship management</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        <VendorMetricCard title="Preferred Vendors" value={vendors.filter(v => v.status === "Preferred").length.toString()} icon={Award} color="text-amber-500" />
        <VendorMetricCard title="Contracts Ending" value="5" sub="Next 30 days" icon={ShieldCheck} color="text-brand" />
        <VendorMetricCard title="Under Review" value={vendors.filter(v => v.status === "Under Review").length.toString()} sub="Performance audits" icon={AlertCircle} color="text-rose-500" />
      </div>
      ...
      <div className="bg-card-bg border border-border-dim rounded-2xl p-2 sm:p-3 flex flex-col xl:flex-row xl:items-center gap-3 shadow-sm transition-colors duration-300">
         ...
         <div className="flex items-center gap-2 w-full xl:w-auto">
            ...
            <button className="flex items-center justify-center gap-2 px-4 py-2 text-slate-500 hover:text-brand transition-all border border-border-dim rounded-xl xl:border-none xl:rounded-none">
               <Globe className="w-3.5 h-3.5" /> <span className="text-[10px] font-black uppercase tracking-widest">Marketplace</span>
            </button>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredVendors.length === 0 ? (
          <div className="col-span-full py-12 sm:py-20 text-center">
             ...
             <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest">No vendors matching criteria</p>
          </div>
        ) : (
          filteredVendors.map(vendor => (
            ...
            >
               ...
               <div className="mt-6">
                  <span className="text-[9px] font-black text-brand uppercase tracking-tighter font-mono">{vendor.id}</span>
                  <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-brand transition-colors truncate">{vendor.name}</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-l-2 border-brand/30 pl-2 mt-1 truncate">{vendor.category}</p>
               </div>
               ...
               <div className="mt-6 sm:mt-8 grid grid-cols-2 gap-4">
                  ...
                  <div className="text-right flex flex-col justify-end">
                     <span className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 sm:mb-1">Total spend</span>
                     <span className="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tighter">{vendor.totalSpend}</span>
                  </div>
               </div>
            </motion.div>
          ))
...
function VendorMetricCard({ title, value, sub, icon: Icon, color }: any) {
  return (
    <div className="bg-card-bg border border-border-dim p-5 sm:p-7 rounded-[1.5rem] sm:rounded-[2.5rem] flex items-center gap-4 sm:gap-6 shadow-sm group hover:scale-[1.02] transition-all">
       <div className={cn("w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center bg-app-bg border border-border-dim shadow-inner", color)}>
          <Icon className="w-6 h-6 sm:w-8 sm:h-8" />
       </div>
       <div>
          <h4 className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">{title}</h4>
          <div className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tighter">{value}</div>
          {sub && <p className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase mt-0.5 tracking-tighter">{sub}</p>}
       </div>
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    "Preferred": "bg-amber-500/10 text-amber-500 border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.05)] text-white bg-amber-500 border-none",
  ...
  return (
    <span className={cn("px-2.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border", styles[status])}>
      {status}
    </span>
  );
}
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-app-bg border border-border-dim w-full max-w-xl rounded-[2.5rem] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] relative">
        ...
        <div className="p-10 space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Vendor Name</label>
            <input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim focus:ring-1 focus:ring-brand outline-none" placeholder="e.g. Acme Corporation" />
          </div>
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Category</label>
              <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim outline-none cursor-pointer">
            ...
            <div className="space-y-2">
              ...
              <input value={formData.contactPerson} onChange={e => setFormData({...formData, contactPerson: e.target.value})} className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim outline-none" />
            </div>
          </div>
          <div className="flex flex-col gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Email</label>
              <input value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim outline-none" placeholder="contact@vendor.com" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Phone</label>
              <input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-card-bg p-4 rounded-2xl text-sm font-bold border border-border-dim outline-none" placeholder="+880..." />
            </div>
          </div>
          <button onClick={() => onSave(formData)} className="w-full py-5 bg-brand text-white font-black rounded-2xl shadow-2xl shadow-brand/30 active:scale-95 transition-all uppercase tracking-[0.2em] text-xs mt-6">
...
function VendorDetailModal({ vendor, onClose, onEdit, onDelete, onUpdate }: any) {
  const navigate = useNavigate();
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl p-0 sm:p-4">
      <motion.div initial={{ scale: 0.95, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 30 }} className="bg-app-bg border border-border-dim w-full max-w-5xl rounded-none sm:rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row h-full max-h-full sm:max-h-[90vh] relative">
         {/* Close Button */}
         <button 
           onClick={onClose}
         ...
         <div className="w-full md:w-96 border-b md:border-b-0 md:border-r border-border-dim bg-slate-50/30 dark:bg-black/20 p-6 sm:p-12 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight leading-none">{vendor.name}</h3>
            <p className="text-[8px] sm:text-[10px] font-black text-brand uppercase tracking-widest mt-2">{vendor.category} PROVIDER</p>
            ...
            <div className="w-full mt-6 sm:mt-10 space-y-2 sm:space-y-3">
               <div className="flex items-center justify-between p-3 sm:p-5 bg-app-bg border border-border-dim rounded-xl sm:rounded-2xl">
                  <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase">Vendor ID</span>
                  <span className="text-[8px] sm:text-[10px] font-black text-slate-900 dark:text-white font-mono">{vendor.id}</span>
               </div>
               <div className="flex flex-col gap-2 p-3 sm:p-5 bg-app-bg border border-border-dim rounded-xl sm:rounded-2xl text-left">
                  <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase mb-1">Status Policy</span>
                  <select 
                    value={vendor.status}
                    onChange={(e) => onUpdate({...vendor, status: e.target.value})}
            ...
            <div className="mt-6 md:mt-auto flex flex-col gap-2 sm:gap-3 w-full">
               ...
               <div className="flex gap-2 sm:gap-3">
                  <button onClick={onEdit} className="flex-1 py-3 sm:py-4 bg-brand text-white font-black rounded-xl sm:rounded-2xl shadow-lg border border-brand uppercase tracking-widest text-[9px] sm:text-[10px] flex items-center justify-center gap-1">
                     <Edit3 className="w-3 h-3 sm:w-4 sm:h-4" /> Edit
                  </button>
                  <button onClick={onDelete} className="flex-1 py-3 sm:py-4 bg-rose-500/10 text-rose-500 border border-rose-500/20 font-black rounded-xl sm:rounded-2xl hover:bg-rose-500 hover:text-white transition-all uppercase tracking-widest text-[9px] sm:text-[10px] flex items-center justify-center gap-1">
         ...
         <div className="flex-1 overflow-y-auto p-6 sm:p-16 space-y-8 sm:space-y-12 custom-scrollbar">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
               <PerformanceMetric label="Quality" value={vendor.metrics.quality} icon={ShieldCheck} color="text-emerald-500" />
               <PerformanceMetric label="Speed" value={vendor.metrics.speed} icon={Truck} color="text-brand" />
               <PerformanceMetric label="Price" value={vendor.metrics.price} icon={DollarSign} color="text-amber-500" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
               <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="space-y-4 sm:space-y-6 bg-slate-50/50 dark:bg-white/5 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-border-dim">
                     <div>
                        <label className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest">Primary Liaison</label>
                        <p className="text-lg sm:text-xl font-black text-slate-900 dark:text-white mt-1 uppercase tracking-tight">{vendor.contactPerson}</p>
                     </div>
                     <div>
                        <label className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest">Office HQ</label>
                        <p className="text-xs sm:text-sm font-bold text-slate-600 dark:text-slate-400 mt-1 uppercase leading-snug">{vendor.location}</p>
                     </div>
                     <div className="flex flex-col gap-3 sm:gap-4">
                        <div className="flex items-center gap-3">
                           <div className="w-7 h-7 sm:w-8 sm:h-8 bg-brand/10 rounded-lg flex items-center justify-center"><Phone className="w-3.5 h-3.5 text-brand" /></div>
                           <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-white">{vendor.phone}</p>
                        </div>
                        <div className="flex items-center gap-3">
                           <div className="w-7 h-7 sm:w-8 sm:h-8 bg-brand/10 rounded-lg flex items-center justify-center"><Mail className="w-3.5 h-3.5 text-brand" /></div>
                           <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-white truncate">{vendor.email}</p>
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="p-6 sm:p-8 bg-brand/5 border border-brand/20 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-inner">
                     <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="px-2 py-0.5 bg-emerald-500 text-white text-[7px] sm:text-[8px] font-black rounded-full uppercase tracking-widest">Active</span>
                     </div>
                     ...
                     <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-brand/20">
                        <div className="flex items-center justify-between">
                           <span className="text-[8px] sm:text-[9px] font-black text-brand/60 uppercase">Spend</span>
                           <span className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">{vendor.totalSpend}</span>
                        </div>
                     </div>
                  </div>
               </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-border-dim pb-4">
                   <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Supply Chain Interaction Log</h4>
                   <button className="text-[10px] font-black text-brand uppercase tracking-widest flex items-center gap-1 hover:underline">View Ledger <ExternalLink className="w-3 h-3" /></button>
                </div>
                <div className="grid grid-cols-1 gap-4">
                   {[
                     ...
                     <div key={i} className="flex items-center justify-between p-6 bg-app-bg border border-border-dim rounded-[1.5rem] hover:bg-slate-50 dark:hover:bg-white/5 transition-all group/item cursor-default">
                        <div className="flex items-center gap-5">
                           ...
                           <div>
                              <span className="text-[8px] font-black text-slate-400 uppercase font-mono tracking-tighter">{tx.id}</span>
                              <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{tx.title}</p>
                           </div>
                        </div>
                        <div className="text-right">
                           <p className="text-base font-black text-slate-900 dark:text-white tracking-tighter">{tx.amount}</p>
...
function PerformanceMetric({ label, value, icon: Icon, color }: any) {
  return (
    <div className="bg-card-bg border border-border-dim p-6 rounded-[2rem] flex flex-col items-center gap-4 text-center shadow-sm">
       <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center bg-app-bg border border-border-dim shadow-inner", color)}>
          <Icon className="w-6 h-6" />
       </div>
       <div className="space-y-1">
          <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{label}</h5>
