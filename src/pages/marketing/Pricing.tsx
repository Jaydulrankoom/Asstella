      ...
      <div className="max-w-7xl mx-auto space-y-20">
        <div className="text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-4 mt-12">
            <span className={cn("text-xs font-black uppercase tracking-widest transition-opacity", !isAnnual ? "text-slate-900 dark:text-white" : "text-slate-400")}>Monthly</span>
            <button 
              onClick={() => setIsAnnual(!isAnnual)}
              className="w-14 h-8 bg-slate-200 dark:bg-white/10 rounded-full p-1 relative transition-all"
            >
              <div className={cn("w-6 h-6 bg-brand rounded-full transition-transform", isAnnual ? "translate-x-6" : "translate-x-0")} />
            </button>
            <span className={cn("text-xs font-black uppercase tracking-widest transition-opacity", isAnnual ? "text-slate-900 dark:text-white" : "text-slate-400")}>Annual <span className="text-emerald-500 ml-1">(20% OFF)</span></span>
          </div>
        </div>
          ...
          {plans.map((plan, idx) => (
            ...
            >
              <div className="space-y-2 mb-8">
                <span className={cn("text-[10px] font-black uppercase tracking-[0.2em]", plan.featured ? "text-brand" : "text-slate-400")}>{plan.badge}</span>
              ...
              <div className="mb-8">
                <div className="flex items-baseline gap-1">
                  ...
                  <span className={cn("text-xs font-black uppercase tracking-widest pl-2", plan.featured ? "text-slate-400" : "text-slate-500")}>/ per month</span>
                </div>
              </div>

              <p className={cn("text-sm font-medium leading-relaxed mb-10", plan.featured ? "text-slate-400" : "text-slate-500")}>{plan.desc}</p>
        ...
        <div className="bg-slate-50 dark:bg-white/5 rounded-[3rem] p-12 md:p-20 border border-slate-100 dark:border-white/5">
           <div className="flex flex-col md:flex-row gap-12 items-start">
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-12">
                 <Question title="Is data exported easily?" desc="Yes, export everything via CSV, PDF, or direct API stream anytime." />
                 <Question title="Can I change plans?" desc="Seamlessly upgrade or downgrade with prorated credits instantly." />
                 <Question title="Do you offer training?" desc="Business and Enterprise plans include full virtual onboarding sessions." />
                 <Question title="Is there an API?" desc="Yes, a robust REST API with 100+ endpoints for deep integration." />
              </div>
           </div>
        </div>
      </div>
...
function Question({ title, desc }: { title: string, desc: string }) {
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed uppercase tracking-tighter">{desc}</p>
    </div>
  );
}
