...
const posts = [
  {
    ...
    excerpt: "How AI is changing the way enterprises manage their sprawling infrastructure ecosystems.",
  ...
  },
  {
    ...
    excerpt: "The hidden costs of manual asset tracking in higher education and the cloud-based solution.",
    author: "Marcus Chen",
    date: "April 28, 2024",
  ...
  },
  {
    ...
    excerpt: "How to build an immutable log that satisfies even the most rigorous global audit standards.",
    author: "David Kim",
    date: "April 15, 2024",
    image: "https://images.unsplash.com/photo-1454165833767-027ff3302bc5?auto=format&fit=crop&q=80&w=800",
    category: "Compliance"
  }
];
      ...
      <div className="max-w-7xl mx-auto space-y-24">
        ...
        <div className="max-w-3xl space-y-6">
          ...
          <p className="text-slate-500 font-medium text-lg max-w-xl">
             Deep dives into infrastructure, capital forecasting, and the future of physical enterprise.
          </p>
        </div>

        {/* Featured Post */}
        <Link to="/blog/1" className="group block relative aspect-[21/9] rounded-[3rem] overflow-hidden shadow-2xl border border-slate-200 dark:border-white/10">
           <img src={posts[0].image} alt="" className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-all duration-1000" />
           <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/40 to-transparent" />
           <div className="absolute bottom-0 left-0 p-12 md:p-20 space-y-6 max-w-3xl">
              <span className="px-4 py-1.5 bg-brand text-white font-black text-[10px] uppercase tracking-widest rounded-full">{posts[0].category}</span>
           ...
           {posts.slice(1).map(post => (
             <Link key={post.id} to={`/blog/${post.id}`} className="group space-y-8">
                <div className="relative aspect-video rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-white/10 shadow-sm">
                   <img src={post.image} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-all duration-1000" />
        ...
        <div className="bg-slate-900 p-16 md:p-32 rounded-[4rem] text-center space-y-10 relative overflow-hidden">
           ...
           <div className="relative z-10 max-w-2xl mx-auto space-y-8 text-white">
              ...
              <h3 className="text-4xl md:text-6xl font-black uppercase tracking-tighter">Stay Ahead of <span className="text-brand">Depreciation.</span></h3>
              <p className="text-slate-400 font-medium">Get quarterly insights into industrial management trends and capital planning best practices.</p>
              <div className="flex flex-col sm:flex-row gap-4">
                 <input type="email" placeholder="ENTER WORK EMAIL" className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-brand uppercase tracking-widest" />
                 <button className="px-10 py-5 bg-brand text-white font-black rounded-2xl shadow-xl hover:scale-105 transition-all text-xs uppercase tracking-widest">Subscribe</button>
              </div>
           </div>
        </div>
      </div>
