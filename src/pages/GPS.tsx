...
export default function GPSPage() {
  ...
  return (
    <div
      className="h-full flex flex-col space-y-6 animate-in slide-in-from-bottom-4 duration-500 overflow-hidden"
      id="gps_page"
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between shrink-0 gap-6">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">
            Real-time asset tracking & fleet telematics
          </p>
        </div>
          ...
          <button
            className={cn(
              ...
              !showMap
                ...
                : "bg-app-bg border-border-dim text-slate-500 hover:text-slate-900 dark:hover:text-white",
            )}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="text-[10px] font-black text-brand uppercase tracking-tighter font-mono truncate">
                        {v.id}
                      </div>
                      <div className="text-sm font-black text-slate-900 dark:text-white group-hover:text-brand transition-colors truncate">
                        {v.name}
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-4 flex items-center justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
                  <div className="flex items-center gap-1.5">
                    <Navigation className="w-3.5 h-3.5 text-brand" /> {v.speed}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Activity className="w-3.5 h-3.5" /> {v.lastSeen}
                  </div>
                </div>
              </div>
            ))}
            ...
            {showMap ? (
              ...
              >
                ...
                <div className="absolute top-4 right-4 sm:top-6 sm:right-6 flex flex-col gap-2 z-10">
                  <button className="bg-white dark:bg-slate-900 p-2 sm:p-3 rounded-xl sm:rounded-2xl shadow-xl border border-border-dim text-slate-600 dark:text-white hover:text-brand transition-colors">
                    <Layers className="w-5 h-5" />
                  </button>
                  <button className="bg-white dark:bg-slate-900 p-2 sm:p-3 rounded-xl sm:rounded-2xl shadow-xl border border-border-dim font-black text-slate-600 dark:text-white hover:text-brand transition-colors text-xs sm:text-base">
                    +
                  </button>
                  <button className="bg-white dark:bg-slate-900 p-2 sm:p-3 rounded-xl sm:rounded-2xl shadow-xl border border-border-dim font-black text-slate-600 dark:text-white hover:text-brand transition-colors text-xs sm:text-base">
                    -
                      ...
                      >
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:gap-12">
                          <div className="w-full sm:w-auto space-y-2">
                            <label className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest block sm:text-center">
                              Fuel Level
                            </label>
                            <div className="flex items-center gap-3">
                              <div className="flex-1 sm:w-32 h-2.5 sm:h-3 bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden border border-border-dim">
                                <motion.div
                                  ...
                                  className={cn(
                                    ...
                                    parseInt(selectedVehicle.fuel) < 20
                                      ? "bg-rose-500"
                                      : "bg-brand",
                                  )}
                                />
                          ...
                          <div className="flex gap-6 sm:gap-8 justify-between w-full sm:w-auto">
                            <div>
                              ...
                              <div className="text-base sm:text-xl font-black text-slate-900 dark:text-white font-mono">
                            ...
                            <div>
                              ...
                              <div className="text-[9px] sm:text-xs font-bold text-slate-500 max-w-[120px] sm:max-w-[150px] truncate">
              ...
              >
                ...
                <div className="space-y-3">
                  <h3 className="text-xl sm:text-3xl font-black tracking-tight uppercase">
                    Fleet Telematics Analytics
                  </h3>
                  <p className="text-slate-500 max-w-sm mx-auto text-xs sm:text-sm font-medium leading-relaxed px-4">
                    Select a vehicle from the sidebar or click beneath to view
                    high-precision positioning data.
                  </p>
                </div>
                <button
                  className="px-8 sm:px-10 py-3 sm:py-4 bg-brand text-white font-black rounded-xl sm:rounded-2xl shadow-2xl shadow-brand/30 uppercase tracking-widest active:scale-95 transition-all text-[10px] sm:text-xs"
                  onClick={() => setShowMap(true)}
                >
                  Switch to Live Map
                </button>
