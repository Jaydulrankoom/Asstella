...
const reportTemplates = [
  { id: "RL-001", name: "Annual Depreciation Schedule", type: "Financial", status: "Ready" },
  { id: "RL-002", name: "Q2 Procurement Audit", type: "Procurement", status: "Recent" },
  { id: "RL-003", name: "IT Hardware Lifecycle", type: "Infrastructure", status: "Scheduled" },
];

export default function ReportsPage() {
  const [activeTab, setActiveTab] = useState("overview");
  const [dateRange, setDateRange] = useState("This Year");
  const [customDates, setCustomDates] = useState({ start: "2024-01-01", end: "2024-12-31" });
  ...
  const kpis = useMemo(() => {
    const multi = dateRange === "Last 30 Days" ? 0.2 : dateRange === "Last Quarter" ? 0.5 : 1;
    return {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500" id="reports_analytics_page">
      {/* Header */}
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
        <div className="flex-1">
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">Advanced lifecycle analytics and expenditure modeling</p>
        </div>
          ...
          <div className="flex flex-1 sm:flex-none bg-card-bg border border-border-dim rounded-2xl p-1 gap-1">
            <button 
              ...
              className={cn(
                ...
                activeTab === "overview" ? "bg-brand text-white shadow-lg" : "text-slate-500 hover:text-slate-900"
              )}
            >
              Overview
            </button>
            <button 
              ...
              className={cn(
                "flex-1 sm:flex-none px-4 sm:px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all",
                activeTab === "financial" ? "bg-brand text-white shadow-lg" : "text-slate-500 hover:text-slate-900"
              )}
            >
              Financials
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <KPICard title="Total Asset Value" value={`$ ${kpis.value}M`} trend="+4.2%" sub="Active appraisal" icon={DollarSign} />
        <KPICard title="Avg Asset Age" value={`${kpis.age} Yrs`} trend="-0.5" sub="Lifecycle health" icon={Activity} variant="success" />
        <KPICard title="Maintenance Ratio" value={`${kpis.ratio}%`} trend="+1.2%" sub="Total upkeep vs value" icon={Clock} variant="warning" />
        <KPICard title="Compliance Score" value={`${kpis.score}/100`} trend="+0.8%" sub="Audit readiness" icon={ShieldCheck} variant="success" />
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === "overview" && (
          ...
          >
             ...
             <div className="lg:col-span-2 space-y-8">
                <ChartWrapper title="Asset Acquisition Trends" sub={`Data range: ${dateRange === "Custom Range" ? `${customDates.start} to ${customDates.end}` : dateRange}`}>
                  <div className="h-[400px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={filteredAcquisitionData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                        <defs>
                          <linearGradient id="colorHard" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#DE3B40" stopOpacity={0.3}/>
                            <stop offset="95%" stopColor="#DE3B40" stopOpacity={0}/>
                          </linearGradient>
                        ...
                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 800, fill: '#64748b' }} tickFormatter={(val) => `$${val/1000}k`} />
                        <Tooltip 
                           contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '12px', color: '#fff', fontSize: '12px' }} 
                           itemStyle={{ color: '#fff', fontWeight: 900 }}
                        />
                        <Area type="monotone" dataKey="hardware" stroke="#DE3B40" strokeWidth={4} fillOpacity={1} fill="url(#colorHard)" />
                        <Area type="monotone" dataKey="software" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorSoft)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </ChartWrapper>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <ChartWrapper title="Maintenance Volume" sub="Weekly service ticket load">
                     <div className="h-[250px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={maintenanceTrends}>
                            ...
                            <YAxis hide />
                            <Tooltip cursor={{ fill: 'rgba(222, 59, 64, 0.05)' }} contentStyle={{ borderRadius: '10px', fontSize: '11px' }} />
                            <Bar dataKey="count" fill="#DE3B40" radius={[6, 6, 0, 0]} />
                          </BarChart>
                        </ResponsiveContainer>
                     </div>
                   </ChartWrapper>

                   <ChartWrapper title="Audit Status" sub="Compliance verification mapping">
             ...
             <div className="space-y-6">
                <div className="bg-slate-900 text-white rounded-[2.5rem] p-10 relative overflow-hidden shadow-2xl">
                   ...
                   <div className="relative z-10">
                      ...
                      <h3 className="text-2xl font-black uppercase tracking-tight leading-none mb-2">Report Export Hub</h3>
                      <p className="text-slate-400 text-xs font-bold leading-relaxed mb-8">Generate certified data exports for external audits and stakeholder management.</p>
        ...
        {activeTab === "financial" && (
          <motion.div 
            ...
            className="space-y-8"
          >
            <ChartWrapper title="Financial Appraisal History" sub="Book value vs replacement cost projection">
               <div className="h-[400px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={filteredAcquisitionData}>
                      ...
                      <Legend />
                      <Bar dataKey="hardware" name="Asset Value" fill="#DE3B40" radius={[6, 6, 0, 0]} />
                      <Bar dataKey="software" name="Depreciation" fill="#10b981" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
               </div>
            </ChartWrapper>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
               <div className="bg-card-bg border border-border-dim rounded-[2.5rem] p-10">
                  ...
                  <div className="space-y-4">
                     {[
                        { name: "SLM Depreciation", rate: "12%", val: "$ 14,200/mo" },
                        { name: "WTM Allowance", rate: "18%", val: "$ 22,500/mo" },
                        { name: "Tax Rebate (Est)", rate: "5%", val: "$ 5,400/mo" }
                     ].map((item, i) => (
                        <div key={i} className="flex items-center justify-between p-6 bg-app-bg border border-border-dim rounded-2xl">
                           ...
                           <div className="text-right">
                              ...
                              <p className="text-[10px] font-bold text-emerald-500 uppercase">{item.rate} EFFICIENCY</p>
                           </div>
                        </div>
                     ))}
                  </div>
               ...
               <div className="bg-slate-900 rounded-[2.5rem] p-12 text-white flex flex-col justify-center text-center">
                  ...
                  <h3 className="text-3xl font-black uppercase tracking-tight mb-4">Optimise Your Taxes</h3>
                  <p className="text-slate-400 text-sm font-medium leading-relaxed mb-10 max-w-sm mx-auto">Our AI models suggest that switching to WTM depreciation for IT assets could save you up to $45k this fiscal year.</p>
...
function KPICard({ title, value, trend, sub, icon: Icon, variant }: any) {
  return (
    <div className="bg-card-bg border border-border-dim p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] shadow-xl relative overflow-hidden group hover:scale-[1.02] transition-all">
      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6 relative z-10">
        ...
        {trend && (
          <span className={cn(
             ...
             trend.startsWith('+') ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" : "text-amber-500 bg-amber-500/10 border-amber-500/20"
          )}>
            {trend}
          </span>
        )}
      ...
      <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tighter relative z-10">{value}</div>
      {sub && <div className="text-[9px] sm:text-[10px] text-slate-500 font-bold mt-2 uppercase tracking-widest opacity-60 underline decoration-brand/20">{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div className="bg-card-bg border border-border-dim rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-10 shadow-sm relative overflow-hidden group">
       <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-10 border-b border-border-dim pb-6 gap-4">
          <div>
             <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{title}</h3>
             <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{sub}</p>
          </div>
          <div className="flex items-center gap-2">
             <button className="p-2 hover:bg-slate-50 dark:hover:bg-white/5 rounded-lg transition-all text-slate-400"><Printer className="w-4 h-4" /></button>
             <button className="p-2 hover:bg-slate-50 dark:hover:bg-white/5 rounded-lg transition-all text-slate-400"><Share2 className="w-4 h-4" /></button>
          </div>
       </div>
       {children}
    </div>
...
function ActivityItem({ user, action, time, icon: Icon }: any) {
  return (
    <div className="flex items-center gap-4 group/item">
       ...
       <div className="flex-1">
          <div className="text-[11px] font-black text-slate-900 dark:text-white uppercase leading-tight">{action}</div>
