...
const initialHardwareDevices: ScannerDevice[] = [
  { id: "SCAN-001", name: "Zebra DS2278", type: "Bluetooth", brand: "Zebra Technologies", status: "Connected", battery: 85, lastUsed: "10m ago" },
  { id: "SCAN-002", name: "Honeywell Voyager", type: "USB", brand: "Honeywell", status: "Disconnected" },
  { id: "SCAN-003", name: "Datalogic Gryphon", type: "USB", brand: "Datalogic", status: "Connected", lastUsed: "1h ago" },
];
...
export default function ScannerPage() {
  ...
  const handlePairing = (id: string) => {
    ...
    setTimeout(() => {
      setDevices(prev => prev.map(d => d.id === id ? { ...d, status: "Connected", battery: 100, lastUsed: "Just now" } : d));
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500" id="scanner_integration_page">
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase flex items-center gap-3">
             <QrCode className="w-7 h-7 sm:w-8 sm:h-8 text-brand" /> Scan & Identify
          </h2>
        ...
        <div className="bg-card-bg border border-border-dim rounded-2xl p-1 flex items-center shadow-sm w-full lg:w-auto">
           ...
           <button
             onClick={() => { setActiveMode("hardware"); setIsScanning(false); }}
             className={cn(
               "flex-1 lg:flex-none px-4 sm:px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2",
               activeMode === "hardware" 
                 ? "bg-brand text-white shadow-lg shadow-brand/20" 
               ...
               >
                  ...
                  {isScanning ? (
                    <div className="w-full max-w-md relative z-10">
                       <div id="qr-reader" className="w-full rounded-2xl overflow-hidden border-4 border-brand/50 shadow-[0_0_50px_rgba(26,63,240,0.2)]"></div>
                       <button 
                    ...
                    <div className="text-center space-y-6 sm:space-y-8 animate-in zoom-in-95 duration-300 relative z-10">
                       ...
                       <div>
                          <h3 className="text-white text-2xl sm:text-3xl font-black uppercase tracking-tight">Identity Verified</h3>
                          <p className="text-slate-400 font-bold uppercase tracking-widest mt-2">{scannedResult}</p>
                       </div>
                       <div className="flex flex-col sm:flex-row gap-4">
                          <button 
                            onClick={() => navigate(`/assets?search=${scannedResult}`)}
                    ...
                    <div className="text-center space-y-6 sm:space-y-8 relative z-10">
                       ...
                       <div className="max-w-xs mx-auto">
                          <h3 className="text-white text-xl sm:text-2xl font-black uppercase">Mobile Lens Active</h3>
                          <p className="text-slate-400 text-[10px] font-bold leading-relaxed uppercase tracking-widest mt-4">Use your device camera to scan asset QR codes for instant verification.</p>
                       </div>
                       <button 
               ...
               >
                  <div className="bg-card-bg border border-border-dim rounded-[2.5rem] p-10 shadow-xl overflow-hidden relative">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                       <div>
                          <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Connected Peripherals</h3>
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Found {devices.filter(d => d.status === "Connected").length} active hardware controllers</p>
                       </div>
                    ...
                    <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
                       {devices.map((device) => (
                         <div key={device.id} className="bg-app-bg border border-border-dim rounded-[2rem] p-8 hover:border-brand/40 transition-all group">
                            <div className="flex items-start justify-between mb-6">
                               <div className={cn(
                                 "w-12 h-12 rounded-xl flex items-center justify-center shadow-lg transition-all",
                                 device.status === "Connected" ? "bg-brand text-white" : "bg-slate-100 dark:bg-white/5 text-slate-400"
                               )}>
                                  {device.type === "Bluetooth" ? <Bluetooth className="w-6 h-6" /> : <Usb className="w-6 h-6" />}
                               </div>
                               <div className="flex flex-col items-end gap-2">
                                  <span className={cn(
                                    "px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-[0.1em]",
                                    device.status === "Connected" ? "bg-emerald-500/10 text-emerald-500" :
                                    device.status === "Pairing" ? "bg-brand/10 text-brand animate-pulse" :
                                  ...
                                  {device.battery !== undefined && (
                                    <div className="flex items-center gap-1.5">
                                       <Battery className={cn("w-3 h-3", device.battery < 20 ? "text-rose-500" : "text-emerald-500")} />
                                       <span className="text-[9px] font-black text-slate-500">{device.battery}%</span>
                                    </div>
                                  )}
                               </div>
                            </div>
                            <h4 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tighter truncate">{device.name}</h4>
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1 mb-6">{device.brand}</p>
                       ...
                       <button className="border-2 border-dashed border-border-dim rounded-[2rem] p-8 flex flex-col items-center justify-center gap-4 hover:bg-brand/5 hover:border-brand/40 transition-all group">
                          ...
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-brand transition-colors">Add Third-Party Controller</span>
                       </button>
                    </div>
                  </div>
               </motion.div>
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="bg-card-bg border border-border-dim rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-sm">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center justify-between">
                 Scanner Engine Status <Activity className="w-4 h-4 text-emerald-500" />
              </h3>
              <div className="space-y-6">
                 <DiagnosticItem label="Decryption Core" status="Operational" value="v4.2.1" icon={ShieldCheck} />
                 <DiagnosticItem label="Network Relay" status="Connected" value="12ms Latency" icon={Wifi} />
                 <DiagnosticItem label="Battery (Phone)" status="Healthy" value="94%" icon={Battery} />
                 <DiagnosticItem label="Lens Calibration" status="Precision" value="Auto-focus" icon={Maximize2} />
              </div>
              <button 
           ...
           <div className="bg-slate-900 rounded-[1.5rem] sm:rounded-[2.5rem] p-8 sm:p-10 text-white relative overflow-hidden group">
              ...
              <h4 className="text-lg sm:text-xl font-black uppercase tracking-tight mb-2 leading-none">External Feed</h4>
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-6">Real-time identification log</p>
              
              <div className="space-y-4 max-h-[220px] overflow-y-auto custom-scrollbar pr-2">
                 <LogEntry time="2m ago" code="IT-SRV-992" user="System" status="VERIFIED" />
                 <LogEntry time="14m ago" code="W-HK-102" user="Admin" status="AUDITED" />
                 <LogEntry time="1h ago" code="VEH-2291" user="Joshua" status="LOCATED" />
                 <LogEntry time="2h ago" code="LAB-PC-44" user="Sarah" status="VERIFIED" />
              </div>
           </div>
           ...
           <div className="bg-brand text-white rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-2xl shadow-brand/30">
              <div className="flex items-center gap-4 mb-4">
                 ...
                 <h4 className="text-base sm:text-lg font-black uppercase">Rapid Audit</h4>
              </div>
              <p className="text-[10px] font-bold text-white/70 uppercase tracking-widest leading-relaxed mb-6">Enable rapid audit mode to skip confirmation dialogs and bulk scan assets at 3x speed.</p>
  ...
  return (
    <div className="flex items-center justify-between group">
       <div className="flex items-center gap-4">
          ...
          <div>
             <div className="text-[9px] font-black text-slate-400 uppercase leading-none mb-1">{label}</div>
             <div className="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-tight">{status}</div>
          </div>
       </div>
       <span className="text-[9px] font-bold text-slate-500 uppercase">{value}</span>
    </div>
  );
}
...
function LogEntry({ time, code, user, status }: any) {
  return (
    <div className="p-4 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-between group">
       <div>
          <div className="text-[11px] font-black text-slate-200 uppercase tracking-tight">{code}</div>
          <div className="text-[8px] font-bold text-slate-500 uppercase mt-0.5">{user} • {time}</div>
       </div>
       <span className={cn(
         "px-2 py-0.5 rounded-[4px] text-[8px] font-black tracking-widest",
         status === "VERIFIED" ? "bg-brand/20 text-brand" : "bg-emerald-500/20 text-emerald-500"
       )}>
          {status}
       </span>
    </div>
