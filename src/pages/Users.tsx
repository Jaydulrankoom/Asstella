...
export default function UsersPage() {
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchesFilter =
      filter === "all" || u.status.toLowerCase() === filter.toLowerCase();
  ...
  return (
    <div
      className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 p-2 sm:p-0"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mt-1">
            Orchestrate enterprise directory and access control
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="bg-card-bg border border-border-dim rounded-[1.5rem] sm:rounded-[3rem] shadow-2xl p-4 sm:p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:hidden gap-4">
          {filteredUsers.map((user) => (
            <div
              key={user.id}
              className="bg-app-bg border border-border-dim rounded-2xl p-5 space-y-4 hover:border-brand/40 transition-all"
            >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-white/5 border-b border-border-dim">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right whitespace-nowrap">
                  Access Operations
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-dim">
              {filteredUsers.map((user) => (
                <tr
                  key={user.id}
                  className="group hover:bg-slate-50 dark:hover:bg-brand/5 transition-all"
                >
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-brand transition-colors">
                          {user.name}
                        </h4>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-black uppercase text-slate-900 dark:text-white tracking-tight">
                        {user.role}
                      </span>
                    </div>
        ...
        <div className="flex items-center justify-between pt-4">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">
            Node capacity: 512/Unlimited
          </p>
          <div className="flex items-center gap-2">
            <button className="p-2 border border-border-dim rounded-xl hover:bg-slate-100 transition-all">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <div className="flex items-center gap-1">
              <span className="w-8 h-8 flex items-center justify-center bg-brand text-white rounded-lg text-xs font-black">
                1
              </span>
              <span className="w-8 h-8 flex items-center justify-center text-slate-500 rounded-lg text-xs font-bold hover:bg-slate-100">
                2
              </span>
            </div>
            <button className="p-2 border border-border-dim rounded-xl hover:bg-slate-100 transition-all">
        ...
        <div className="p-6 sm:p-10 border-b border-border-dim flex items-center justify-between bg-slate-50/50 dark:bg-white/5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">
          ...
          <button
            onClick={onClose}
            className="p-2 sm:p-3 bg-white dark:bg-white/5 border border-border-dim rounded-xl sm:rounded-2xl hover:text-brand transition-all active:scale-95 z-10 text-2xl font-bold"
          >
            ×
          </button>
        </div>
        ...
        <div className="p-6 sm:p-10 space-y-6">
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-medium leading-relaxed mt-0.5">
                  Logged in from Dashboard via OAuth (192.168.1.53)
                </p>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900 dark:text-white">
                  Approved Asset Transfer Request
                </p>
                <p className="text-[10px] text-slate-500 font-medium leading-relaxed mt-0.5">
                  Authorized dispatch of "Dell XPS 15 (TR-892)" to Logistics
                  Dept, London Branch.
                </p>
                <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-2">
                  1 day ago
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-medium leading-relaxed mt-0.5">
                  Resolved critical cooling issue for "HVAC Unit C-22" mapped to
                  Operations floor.
                </p>
                <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-2">
                  3 days ago
      ...
      >
        <div className="p-6 sm:p-10 border-b border-border-dim flex items-center justify-between bg-slate-50/50 dark:bg-white/5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white">
                {user ? "Access Profile" : "New Onboarding"}
              </h3>
              <p className="text-[9px] sm:text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">
                Configure credentials and domain
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 sm:p-10 space-y-6 sm:space-y-8 flex-grow">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                  Approval Manager (Hierarchy)
                </label>
                <select className="w-full bg-app-bg border border-border-dim rounded-[1.2rem] p-4 text-sm focus:ring-4 focus:ring-brand/10 outline-none transition-all font-black uppercase appearance-none text-slate-500">
          ...
          {!user && (
            <div className="bg-brand/5 border border-brand/10 p-4 sm:p-6 rounded-2xl sm:rounded-3xl space-y-4">
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-400">
                Onboarding this user will trigger an automated secure invite via
                the PGP-encrypted mail gateway.
              </p>
            </div>
          )}
        </div>
...
function StatsCard({
  ...
  return (
    <div className="bg-card-bg border border-border-dim rounded-[2rem] p-6 shadow-xl relative overflow-hidden group">
      <div
        className={cn(
          "w-12 h-12 rounded-2xl flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform",
          colors,
        )}
      >
