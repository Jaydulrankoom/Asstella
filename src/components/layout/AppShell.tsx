...
const tenantNavItems: NavItem[] = [
  ...
  {
    label: "Finance & Depreciation",
    href: "/app/finance",
    icon: Coins,
    roles: ["Super Admin", "Company Admin", "Finance Viewer"],
  },
  { label: "Vehicles (GPS)", href: "/app/gps", icon: MapPin },
  {
    ...
    href: "/app/audit",
    icon: ShieldCheck,
    roles: ["Super Admin", "Company Admin", "Asset Manager"],
  },
  { label: "Procurement", href: "/app/procurement", icon: ShoppingCart },
  {
    ...
    href: "/app/vendors",
    icon: Users,
    roles: ["Super Admin", "Company Admin", "Procurement Officer"],
  },
  {
...
const platformNavItems: NavItem[] = [
  { label: "Platform Overview", href: "/app/platform", icon: LayoutDashboard },
  { label: "Tenant Management", href: "/app/platform/tenants", icon: Users },
  { label: "Subscription & Billing", href: "/app/platform/billing", icon: Coins },
  { label: "Plans & Modules", href: "/app/platform/plans", icon: Package },
  { label: "Platform Users", href: "/app/platform/users", icon: UserCog },
  { label: "Integrations Setup", href: "/app/platform/integrations", icon: Share2 },
  { label: "SaaS Analytics", href: "/app/platform/analytics", icon: BarChart3 },
  { label: "Support Helpdesk", href: "/app/platform/support", icon: AlertTriangle },
  { label: "Global Security", href: "/app/platform/security", icon: ShieldCheck },
  { label: "Platform Settings", href: "/app/platform/settings", icon: Settings },
];
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isPlatformUser = currentUser?.role === "Platform Owner";
  const activeNavItems = isPlatformUser ? platformNavItems : tenantNavItems;
  ...
  const notifications = [
    {
      ...
      description: "Generator A-12 requires immediate oil change.",
      type: "urgent",
      time: "2m ago",
    },
    {
      id: 2,
      title: "AMC Expiring",
      description: "Dell Support contract expires in 14 days.",
      type: "warning",
      time: "1h ago",
    },
    {
      id: 3,
      title: "Asset Audited",
      description: "Workstation 405 was successfully verified.",
      type: "info",
      time: "3h ago",
    },
    {
      ...
      description: "HR shared 10 new Laptops with your department.",
      type: "info",
      time: "5h ago",
    },
  ];
    ...
    <>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        {!isPlatformUser && (
          <div
            className={cn(
              "pt-6 pb-2 border-t border-border-dim mt-4",
              collapsed ? "px-0" : "px-4",
            )}
          >
            {!collapsed && (
            ...
            <div className="space-y-1">
              ...
              <QuickAction
                ...
                icon={Wrench}
                href="/app/maintenance?action=create"
                isCollapsed={collapsed}
              />
              <QuickAction
                id="qa_audit"
              ...
              <QuickAction
                ...
                icon={Calculator}
                href="/app/finance?action=depreciate"
                isCollapsed={collapsed}
                roles={["Super Admin", "Company Admin"]}
              />
              <QuickAction
                id="qa_gps_map"
                label="View GPS Live Map"
                icon={Map}
                href="/app/gps?action=map"
                isCollapsed={collapsed}
              />
              <QuickAction
                id="qa_procure"
      ...
      <Link
        to="/app/profile"
        onClick={onNavItemClick}
        className="p-4 mt-auto border-t border-border-dim bg-slate-50 dark:bg-black/20 transition-colors duration-300 hover:bg-slate-100 dark:hover:bg-white/5 group"
      >
        <div
          className={cn(
        ...
        >
          ...
          {!collapsed && (
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-bold truncate group-hover:text-brand transition-colors">
  ...
  return (
    <div
      className="flex h-screen bg-app-bg text-app-text overflow-hidden transition-colors duration-300"
      id="app_root"
    >
      {/* Desktop Sidebar */}
      <aside
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-app-bg transition-colors duration-300">
        {/* Header */}
        <header
          className="sticky top-0 h-16 bg-app-bg/80 backdrop-blur-md border-b border-border-dim flex items-center justify-between px-4 md:px-8 z-40 transition-colors duration-300"
          id="header"
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <h2
                className="text-sm md:text-lg font-bold truncate max-w-[120px] md:max-w-none"
                id="page_title"
              >
                {activeNavItems.find((i) => i.href === location.pathname)?.label ||
                  "Module"}
              </h2>
              <span className="text-[8px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest hidden sm:inline">
                {isPlatformUser ? "Platform Admin" : "Enterprise Edition v2.4"}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 md:gap-6">
            <div className="flex items-center gap-1 md:gap-3 text-slate-400 relative">
              ...
              >
                {theme === "light" ? (
                  <Moon className="w-4 h-4 md:w-5 h-5 text-slate-600" />
                ) : (
                  <Sun className="w-4 h-4 md:w-5 h-5 text-brand" />
                )}
              </button>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="absolute -top-1 -right-1 w-3.5 h-3.5 md:w-4 md:h-4 bg-rose-500 text-[8px] md:text-[10px] text-white flex items-center justify-center rounded-full border-2 border-app-bg font-black">
                    4
                  </span>
                </button>
                    ...
                    >
                      ...
                      <div className="max-h-96 overflow-y-auto custom-scrollbar divide-y divide-border-dim">
                        {notifications.map((notif) => (
                          <div
                            key={notif.id}
                            className="p-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors cursor-pointer group whitespace-normal"
                          >
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-slate-900 dark:text-white truncate group-hover:text-brand transition-colors">
                                  {notif.title}
                                </p>
                                <p className="text-xs text-slate-500 line-clamp-2 mt-0.5 mb-2 leading-relaxed">
                                  {notif.description}
                                </p>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 cursor-pointer group hover:bg-slate-100 dark:hover:bg-white/5 p-1 -m-1 rounded-xl transition-all"
              >
                <img
                  src="https://api.dicebear.com/7.x/avataaars/svg?seed=Sabbir"
                  alt="User"
                  className="w-8 h-8 rounded-full border border-border-dim"
                />
                <div className="hidden xl:flex flex-col text-left">
                  <span className="text-sm font-bold group-hover:text-brand transition-colors">
                    {currentUser?.name || "User"}
                  </span>
              ...
              <AnimatePresence>
                {isProfileMenuOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    className="absolute right-0 mt-3 w-56 bg-app-bg border border-border-dim rounded-2xl shadow-2xl z-50 overflow-hidden"
                  >
                    <div className="p-3 bg-slate-50 dark:bg-white/5 border-b border-border-dim">
                      ...
                      <div className="flex flex-col gap-1">
                        <button
                          onClick={() => {
                            if (currentUser) {
                              setCurrentUser({...currentUser, role: "Platform Owner"});
                            }
                            setIsProfileMenuOpen(false);
                            navigate('/app/platform');
                          }}
                          className={cn("text-left px-3 py-2 text-sm rounded-lg hover:bg-white dark:hover:bg-black/20 transition-all font-medium", currentUser?.role === "Platform Owner" && "bg-brand/10 text-brand")}
                        >
                          Platform Owner
                        </button>
                        <button
                          onClick={() => {
                            if (currentUser) {
                              setCurrentUser({...currentUser, role: "Super Admin"});
                            }
                            setIsProfileMenuOpen(false);
                            navigate('/app');
                          }}
                          className={cn("text-left px-3 py-2 text-sm rounded-lg hover:bg-white dark:hover:bg-black/20 transition-all font-medium", currentUser?.role === "Super Admin" && "bg-brand/10 text-brand")}
                        >
                          Tenant Admin
                        </button>
                      </div>
                    ...
                    <div className="p-2">
                      <Link to="/app/profile" onClick={() => setIsProfileMenuOpen(false)} className="flex w-full items-center px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg text-slate-700 dark:text-slate-300">
                        My Profile
                      </Link>
                      <button className="flex w-full items-center px-3 py-2 text-sm hover:bg-rose-500/10 hover:text-rose-600 rounded-lg text-slate-700 dark:text-slate-300">
                        Sign Out
                      </button>
                    </div>
                  </motion.div>
        ...
        <main
          ref={viewportRef}
          className="flex-1 overflow-y-auto p-2 md:p-3 custom-scrollbar bg-app-bg transition-colors duration-300"
          id="main_viewport"
        >
          {children}
        </main>
              ...
              <div className="p-6 sm:p-12 space-y-6 sm:space-y-8">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-slate-400 text-xs sm:text-sm font-medium mt-2">
                    Position the asset QR code within the frame to scan
                  </p>
                </div>
                ...
                <div className="relative aspect-square max-w-[200px] sm:max-w-[280px] mx-auto group">
                  ...
                  <div className="absolute inset-3 sm:inset-4 overflow-hidden rounded-xl sm:rounded-2xl bg-black flex items-center justify-center text-center">
                    ...
                    <div className="text-white/20 select-none flex flex-col items-center gap-2">
                      ...
                      <span className="text-[8px] sm:text-[10px] font-black tracking-[0.2em] uppercase">
                        Initialising...
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-3 sm:gap-4">
                  <div className="p-3 sm:p-4 bg-white/5 border border-white/5 rounded-xl sm:rounded-2xl">
                    <p className="text-[9px] sm:text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">
                      Last Scan Result
                    </p>
